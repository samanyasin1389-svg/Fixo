...
کاتالوگ اپ‌ها در `~/.config/fixo/apps-catalog.json` ذخیره می‌شود. در UI قبل از نصب، اکانت پلی (**بله/خیر**) پرسیده می‌شود؛ اگر خیر باشد گزینه پلی حذف می‌شود. منابع: خودکار، پلی، جستجوی مدل‌محور (باز کردن مرورگر لپ‌تاپ)، APK محلی، گیت‌هاب، لینک مستقیم. بک‌آپ pause/resume/cancel دارد.
